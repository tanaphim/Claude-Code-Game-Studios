...
| FT12 | World Map (10 Neutral Cities) | ✅ **REVISION COMPLETE — externally-gated items pending (2026-05-05)**. **2 BLOCKING / 33 RECOMMENDED** remaining ; ทั้งคู่ไม่ actionable ใน FT12 alone: (1) **Cluster 5 #13** OQ-10 Fragment routing perverse incentive — gated on FT13/FT14 GDD authoring ; (2) **narr-3 partial** — 8 of 10 city theme briefs pending narrative writer-day allocation (~4 wd, deferable to next sprint). Phase 3 closed **20 of original 28 blockers** via 5 passes (Cluster 4 #12 R2.1 → Cluster 6+2+7 paired → narrative pass → UX pass → Producer OQ-7). AC suite expanded **49 → 62** ; R13.1 execution-environment binding constraints added (Functions Premium + Redis Premium pinned) ; `production/qa/environment-spec.md` authored ; OQ-7 locked to PC-only. Recommended next: full `/design-review` re-run (cold-context adversarial) + engineering pick (Path A telemetry vs Path B PlayFab key) for Cluster 4 #10 state model | design/gdd/world-map-system.md |
