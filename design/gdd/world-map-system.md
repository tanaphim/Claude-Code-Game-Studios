...
2. **Functional orientation (separate mechanism)** — ผู้เล่นต้องรู้ว่า
   ...
   active เฉพาะ first-visit) ที่ open City Menu (Universal Services hub).
   Alternative mechanisms (contextual labels, signage, mini-tutorial
   overlay) defer ไป UX spec.
...
>
> **Coverage:** 49 criteria — Core Rules 15 + 4 (R3.1, R12 atomicity, R12 CAS,
> R15) + 5 additive (R5.1, R9.1, R16 ×3), Formulas 6, Edge Cases 9 + 2
> (EC-14, EC-18) + 1 additive (EC-26 sequencing), Cross-System 3,
> Performance 2 + 1 (TR-042 mid-range overflow), Manual 2 + 1 additive
> (G.5 fragmentation alert). ดู H.7 สำหรับ criteria ที่ defer รอ FT13/FT14/M11.
