...
### Keeper voice profile (closes Cluster 8 #17 + narr-1)

Keeper เป็น **archetype ที่ซ้ำในทุก 10 เมือง** (ไม่ใช่ตัวละครชื่อเดียว) —
แต่ละเมืองมี Keeper เฉพาะตัวที่ยังคง voice register เดียวกัน, art direction
สามารถ vary ตาม city theme ได้ (ดู Visual/Audio Requirements). Keeper พูดด้วย
register ของ **ผู้ดูแลสถานที่เก่าแก่** — รู้จักลานนี้ดีกว่าใครทั้งนั้น แต่
ไม่อวดความรู้ ; สุภาพโดยไม่ต้องประจบ, มีน้ำหนักโดยไม่ต้องเคร่งขรึม. ใช้
ภาษาไทยที่เป็นทางการเล็กน้อยกว่า conversational แต่ไม่ถึงขั้น
formal-bureaucratic.

**Vocabulary ห้าม** สำหรับ Keeper:
- คำเชิญชวนเชิงพาณิชย์ ("ยินดีต้อนรับสู่...")
- ภาษา tutorial ("กดปุ่มเพื่อ...", "คลิกที่นี่")
- คำสรรพนามที่บอกความเป็นเจ้าของลาน ("ลานของฉัน", "เมืองของเรา")
- Military/authoritative register (ฐานทัพ/ยึด/ป้อม/แดนเรา/บุก —
  สอดคล้อง vocabulary rule ของ Section B ด้านบน)

Keeper พูดจาก position ของ **พยาน** ที่เห็นทุกฝ่ายผ่านมา ไม่ใช่ **ไกด์**
ที่นำทาง. Keeper รู้จักชื่อของทุกฝ่ายโดยไม่ถือฝ่ายใด.

**Sample lines (golden — narrative writer pass อนุมัติ):**

| Moment | Sample (Thai) |
|---|---|
| First-visit greeting | *"ลานนี้เปิดรับทุกธง — เคยมีคนจากทุกฝ่ายมายืนตรงที่ท่านยืนอยู่นี้. อะไรที่ท่านต้องการ ก็มีที่นี่."* |
| Post-first-visit greeting | *"ท่านกลับมาแล้ว. ลานยังเหมือนเดิม."* |
| Ambient passive (overhearable) | *"ระฆังจะบอกให้รู้เองเมื่อถึงเวลา."* |
| Bell-time ambient (R8 t=0+0.5s) | *"ได้ยินไหม — ลานร่วมกันรับรู้แล้ว."* |

Bell-time line trigger เป็น **ambient one-shot** หลัง bell audio + avatar
cue ของ R8 ; proximity scope (R10 — เฉพาะผู้เล่นใน Keeper interaction
radius ได้ยิน), ไม่ broadcast city-wide, ไม่ require interact.

**Visual archetype brief (สำหรับ art-director):** Keeper ใน 10 เมืองมี
visual variant ที่ reflect city theme ขณะคง silhouette register เดียวกัน:
ผู้ดูแลในชุดเรียบไม่มีสัญลักษณ์ฝ่าย, สีกลาง (ไม่ใช่สีของ faction ใด), ท่าทาง
สงบ-ไม่ตึงเครียด. Lean welcoming-host มากกว่า lean-ascetic — มีน้ำหนักทาง
กายภาพ (ไม่ใช่ร่างผอมบาง) แต่ไม่ใช่ทหาร. ห้าม armament, ห้าม faction emblem,
ห้าม ornate robe — ชุดเรียบที่บอกว่า "อยู่ที่นี่นาน" ไม่ใช่ "มีอำนาจ".
...
**Avatar behavioral cue at bell time (closes Cluster 1 #1):** ณ t=0+0.3s
(ทันทีหลัง EC-26 sequencing ของ bell audio), avatars ทุกตัวที่อยู่ใน state
`InCity` ของ instance นั้น (รวมผู้เล่นที่กำลังเดินหรือ idle) trigger
animation override ชั่วคราว: **"มองกลับขึ้น" (look-up-toward-Monument)** —
head-turn lerp ไปยัง Monument world position, duration 1.5s, eased-in-out,
**ไม่ lock input** (ผู้เล่นยังขยับ/พิมพ์ได้ระหว่าง animation). Animation
เป็น **additive override** บน locomotion layer — ถ้าผู้เล่นกำลังเดิน
head-turn เกิดบน body ที่กำลังเคลื่อน. **Avatars ที่อยู่ใน modal UI
(City Menu, Tournament Queue ฯลฯ) ณ trigger time:** silent skip (avatar
render บน modal background ไม่ player-facing), ไม่ใช่ error. Cross-instance
synchronicity: cue ต้อง fire ภายใน skew ≤ `R8_CROSS_INSTANCE_SKEW_BUDGET_SECONDS`
(3s) เช่นเดียวกับ bell audio และ Monument transform — fan-out mechanism
เดียวกัน (R13 + R13.1) ครอบ cue นี้ด้วย. Animation rig layer + blend weight
defer ไป animator pass ; GDD commits: primitive ต้องมี + ห้าม lock input +
skew budget เดียวกับ R8 + silent skip สำหรับ modal-covered avatars.
...
Anchor moment ใน Section B ("ทุกคนในจัตุรัสหันมามองกันครู่หนึ่ง") delivered
ผ่าน avatar behavioral cue ข้างต้น — ทุก avatar หันไปยัง Monument พร้อมกัน
ทำให้ผู้เล่นเห็น strangers ในรัศมีของตัวเอง *ทำท่าเดียวกัน* ที่เวลาเดียวกัน,
ปิด collective-witness gap ที่เคย "added the noun, not the verb". Anchor
moment จึงเป็น **city-scoped awareness** (ผ่าน presence flash + simultaneous
avatar cue) + **instance-scoped social** (ผ่าน proximity chat + visual
...
**Hard ceiling concurrency (EC-06 fix + Cluster 2 #5 fix):** D1 step 1
hard-ceiling check ต้อง implement ผ่าน **server-side compare-and-swap (CAS)
ใน Azure Cache for Redis (Premium SKU) ผ่าน Lua script atomic op** —
read-before-write **ไม่เพียงพอ** เพราะมี TOCTOU race: สอง parties (size
16 ต่อกลุ่ม) อ่าน `pop=164` พร้อมกัน, ทั้งคู่คำนวณ `164+16=180 ≤ 180`,
ทั้งคู่ admit, ผลลัพธ์ `pop=196` (ละเมิด ceiling).

**Two CAS systems, two stores (closes Cluster 2 #5):** R12 ใช้ **สอง CAS
systems คนละ store** :
1. **Player Record CAS** = PlayFab user-data per-key version token
   (R12 atomicity contract เดิม). 4 fields ต้อง co-located ใน partition
   เดียว, single `UpdateUserData` call. Per-player write rate ต่ำ ;
   PlayFab plan budget เพียงพอ.
2. **Instance population CAS** = Azure Cache for Redis (`inst-pop:*`
   keyspace, no TTL). Lua script atomic read-claim-write. Sub-ms latency,
   500 RPS+ tolerated. **ห้าม implement ผ่าน PlayFab user-data** เพราะ
   shares plan rate limit (Cluster 3 #9) ; **ห้ามใช้ CosmosDB ETag**
   เพราะ 5–10ms read latency invalidates D1 budget. รายละเอียด
   binding constraint อยู่ใน R13.1.

D1 step 1 ห้ามใช้ plain read-then-write ; ทุก hard-ceiling enforcement
fires CAS via Redis Lua script ที่ reject เมื่อ `pop_after_claim > hard_ceiling`.
...
**Instance roster hot-row mitigation (closes Cluster 2 #6):** `LookupInstance`
และ `ListInstancesOfCity` (D1 step 2 + D2) คือ hot read paths ระหว่าง
Fragment Event spike (EC-20) และ launch-spike (EC-19). Read-through cache
**= same Azure Cache for Redis instance** as instance population CAS,
keyspace prefix `cache:*`, TTL ≤ 5s. Single-shared instance ข้าม Function
workers ทุกตัว — ห้าม per-Function-worker private cache (defeats hot-row
mitigation under scale-out per Cluster 2 #6 trap). Eviction policy =
`volatile-lru` (only TTL'd `cache:*` keys evict ; `inst-pop:*` CAS keys
ไม่หาย). Staleness ≤ 5s ยอมรับได้เพราะ EC-07 freshness window ก็ใช้ 5s.
Binding constraint รายละเอียด อยู่ใน R13.1.

**PlayFab call-count budget (closes Cluster 3 #9 partial):** D1 happy-path
ต้องไม่เกิน **3 PlayFab calls/req** (เดิม 4). Implementation pattern:
combine `LookupInstance` + `TryClaimSlots` เป็น **single CAS call** ที่
carry ทั้ง lookup + claim — read-repair fall-through (instance null)
ทำใน server response handler ของ call เดียวกัน, ไม่ใช่ pre-call lookup.
ที่ cache hit (ListInstancesOfCity ใน step 2 hits 5s TTL cache),
call count ลดลงเหลือ **2 calls/req** (combined-CAS + UpdateUserData).
ที่ launch-spike scale (Scenario E exposure), 3 calls × 500 RPS =
~1500 calls/s ; cache-hit case 2 calls × 500 RPS = ~1000 calls/s ;
ทั้งสอง within "Indie Studio" tier ceiling 2500 calls/s ที่ระบุใน
`production/qa/environment-spec.md`. ห้าม implement R12 ด้วย separate
LookupInstance + TryClaimSlots calls ในลำดับ pre-spike ; pattern นั้น
violate budget และจะ trip rate-limit ที่ launch-mode load.
...
**R13.1 — Execution Environment Binding Constraints (closes Cluster 6 #14 + Cluster 1 #2)**
ตัวเลือก execution environment ของ FT12 ไม่ใช่ deferred-to-ADR — เพราะ
timing budgets ของ D2 (5s tick) และ R8 (1s skew lower bound) เป็น
**pillar-load-bearing** สำหรับ P3 (Fragment Event collective moment) และ
P5 (capacity guarantees ที่ Scenario E launch-spike). ต่อไปนี้คือ
binding constraints ที่ FT12 commit ไว้ ; ห้ามเลื่อนไป R13 ADR.

- **Azure Functions tier = Premium + 3 pre-warmed instances per region**
  (เลือก, ไม่ใช่ defer). Consumption tier 200–800ms cold-start ละเมิด
  D2 5s tick budget (Scenario E exposure) และ R8 1s skew lower bound
  (Cluster 1 #2 same mechanism). 3 pre-warmed = covers D2 scheduler +
  R8 fan-out + general CRUD โดยไม่มี contention ที่ steady state ;
  scale-out ผ่าน Premium plan elastic limits ที่ launch-mode load.
  Region pinning ต้องตรงกับ PlayFab + Photon + Redis (ดู env-spec.md).

- **Instance population CAS store = Azure Cache for Redis (Premium SKU)**
  (เลือก, ไม่ใช่ defer per R12 originally). PlayFab user-data per-key
  version ใช้สำหรับ Player Record CAS (R12 atomicity contract เดิม) ;
  Redis ใช้สำหรับ instance population CAS (TryClaimSlots) — **คนละ store**
  per Cluster 2 #5 fix. Pattern: Lua-script atomic CAS (read-modify-write
  ภายใน single Redis op). Sub-ms latency ; pairs กับ Functions Premium
  ใน same Azure region. ห้ามใช้ CosmosDB ETag (5–10ms read latency
  invalidates 500 RPS budget) ห้ามใช้ PlayFab CloudScript table (shares
  rate limit ที่ Cluster 3 #9 พยายามหลีกเลี่ยง).

- **Read-through cache layer = Same Redis instance with keyspace separation**
  (closes Cluster 2 #6). Single Redis = single operational artifact
  (monitoring + backup + replication). Keyspace prefixes: `inst-pop:*`
  สำหรับ CAS keys (no TTL, live for instance lifetime), `cache:*` สำหรับ
  read-through cache (TTL ≤ 5s per existing R12 contract). Eviction
  policy = `volatile-lru` (only TTL'd keys evict, CAS keys ไม่หาย).
  Single-shared instance ข้าม Function workers ทุกตัว — eliminates
  per-Function-worker private cache trap ของ Cluster 2 #6.

- **Cross-mechanism implications:** R8 fan-out mechanism (Cluster 1 #2)
  ใช้ Premium pre-warm ขจัด cold-start ใน 1s skew budget ; Cluster 6 #14
  D2 tick ได้ guaranteed ≤ 5s ภายใต้ Premium ; Cluster 3 #9 PlayFab rate
  ลดลงเพราะ R12 call budget cap (3 calls/req) + Redis absorbing CAS load
  ที่เคยอยู่บน PlayFab.

ทั้ง 3 commitments ต้อง pin ใน `production/qa/environment-spec.md` ก่อน
performance ACs (TR-034a/b/c, TR-035, TR-042, TR-055) จะ executable.
...
- **1-button targeted acknowledge action** (working name: **"ผ่านลาน /
  Wayfarer's Nod"** — replaces "Traveler's Salute" per narr-2026-05-05
  pass ; military "Salute" register banned per Section B vocabulary
  rule) — กดที่ player target ใน radius `R16_ACKNOWLEDGE_RADIUS_UNIT`
  (default 8, ดู Section G) → trigger reciprocal acknowledge emote
  บนทั้งสองฝั่งพร้อมกัน. **Vocabulary note:** UI label, tooltip, animation
  brief, และ writer pass ต้องใช้ register นบ/วันทา/แสดงคารวะ/ทักทาย —
  ห้ามใช้ military register (salute/march/formation) ทุกที่ใน R16 spec
...
- **EC-11** **If ban event มาถึงขณะ player `InCity`**: state transition →
  `OutOfUniverse` ทันที + force-exit instance + Keeper-voiced broadcast
  ใน instance เท่านั้น (R10 scope) ตาม **3-case differentiation** (closes
  narr-2 — Philosophy B "Diegetic Keeper ejection" chosen, รายละเอียด
  rationale ใน narrative pass review log 2026-05-05) :
  - **Voluntary leave** (Galaxy Map travel หรือ session close):
    *"[ชื่อผู้เล่น] ออกเดินทางจากลานแล้ว"*
  - **Disconnect** (network drop, expected reconnect):
    *"[ชื่อผู้เล่น] หลุดจากลานชั่วคราว"*
  - **Ban** (M11 ส่ง `player_banned`): **ห้ามระบุชื่อผู้ถูก ban**,
    Keeper-voiced phrasing — *"ผู้ดูแลลานได้ขอให้ผู้ถือธงท่านหนึ่งออกจากลาน"*
    (รักษา dignity + ส่ง signal ว่า norm enforced โดย Keeper presence,
    แทน invisible moderation ที่ทำให้ ban กลืนหายไปกับ voluntary leave)

  ทุก broadcast scope = instance only (ไม่ city-wide) — สอดคล้อง R10.
...
- **EC-14** **If network drop ระหว่าง `EnteringCity` load**: client retry
  ...
  `current_instance_id` slot ไว้ `ENTERING_HOLD_SECONDS` (default **30s**
  per Cluster 3 #8 fix — เดิม 60s, ลดเพื่อจำกัด ghost-slot phantom
  occupancy ที่ launch-spike scale per EC-19 ghost-slot accounting) เผื่อ
  reconnect — เกิน → release slot. **Tradeoff acknowledged:** ผู้เล่นที่
  มี long reconnect window จะ miss slot และต้อง re-pick instance ผ่าน
  D1 ใหม่ ; ผลกระทบ minimal เพราะ D1 step 1 co-location + step 2 pack
  รับประกัน assignment ได้ภายใน 1 retry. Knob ยัง live-tunable ผ่าน
  Section G ; ops สามารถ raise ได้ชั่วคราวเพื่อ accommodate ปัญหา network
  ที่ทราบล่วงหน้า แต่ห้ามเกิน 60s โดยไม่มี ghost-slot recompute.
...
- **EC-19** **If launch day flood — 5000+ new players spawn ที่
  `STARTER_CITY_ID` ใน 10 วินาที**: server-side **launch-mode policy**
  pre-warm `STARTER_CITY_INSTANCE_PREWARM_COUNT` (steady-state default 5)
  ก่อน server accept connections. Steady-state 5 รองรับ 750 concurrent
  new players (5 × 150).

  **Launch-mode override (closes Cluster 3 #7):** สำหรับ launch day หรือ
  marketing-push event ที่คาดว่าจะมี ≥1000 concurrent new players ใน
  10s window, ops ต้อง **switch CBS knob เป็น `LAUNCH_MODE_PREWARM_COUNT`
  (default 35 → 5250 capacity)** ก่อน 1 ชม. ของ event start ; revert
  กลับ default 5 ภายใน 24 ชม. หลัง event end. Switching mechanism =
  CBS hot-reload (ไม่ต้อง server restart) ; revert process = ops playbook
  step. ห้าม leave LAUNCH_MODE_PREWARM_COUNT active เกิน 48 ชม. (idle
  resource cost ≈ 30 instances × Photon CCU price + Azure compute).

  **Cohort-tiered SLA (closes Cluster 3 #7 + qa-1):** ที่ steady-state
  prewarm=5 + 5000-player spike, performance contract แยกตาม cohort:
  - **Prewarm cohort (first 750)**: D1 step-3 invoke rate = 0% ;
    p95 instance assignment latency ≤ 200ms ; ไม่มี EC-03 toast
  - **Overflow cohort (751–5000)**: D1 step-3 invoke rate ≥ 80%
    (acknowledged degraded mode) ; p95 latency ≤ 2000ms ; EC-03 toast
    "หนาแน่นชั่วคราว" tolerated สำหรับ ผู้เล่น ที่ hit SpawnNewInstance
    fail (resource exhaust) ; ไม่มี HTTP 5xx ; total spike resolution
    ≤ 60s post-spike-end (5000-player ramp absorbed within 70s of t=0)

  **Ghost-slot accounting (closes Cluster 3 #8):** EC-14 hold contract
  reserves slots ระหว่าง EnteringCity. ที่ disconnect rate ~10% และ
  `ENTERING_HOLD_SECONDS=30` (per Cluster 3 #8 fix), peak ghost slots
  ≈ `0.10 × spike_size × ENTERING_HOLD/spike_window_total`. ที่
  spike=5000, hold=30s, window=10s → ghost peak ≈ 0.10 × 5000 × 30/10
  = 1500 ghost-slot-seconds spread over 30s window = peak ~150 phantom
  occupied slots ที่ t≈10s. Effective capacity sizing ต้อง subtract
  ghost slots: `effective = (prewarm × cap) − peak_ghost_slots`. ที่
  LAUNCH_MODE_PREWARM_COUNT=35 (5250 raw capacity), effective ≈ 5100 —
  ยัง ≥ 5000 spike. ที่ steady prewarm=5 (750 raw), effective ≈ 600 —
  prewarm cohort ลดลงจาก 750 → 600 ; design contract ยัง holds เพราะ
  cohort-tiered SLA แยก prewarm vs overflow.
...
| `ENTERING_HOLD_SECONDS` | **30** | 30 – 60 | Server hold instance slot ระหว่าง player `EnteringCity` reconnect (EC-14). Default ลดลงจาก 60 → 30 per Cluster 3 #8 fix (ghost-slot phantom occupancy). ต่ำกว่า 30 = ละเมิด `ENTERING_TIMEOUT_SECONDS` lower bound (ไม่ valid retry window) ; สูงกว่า 60 = ghost slots ที่ launch-spike scale uncounted ใน EC-19 sizing — ห้าม raise โดยไม่ recompute | ✅ Live-tune (ภายใน safe range) |
...
| `R16_ACKNOWLEDGE_RADIUS_UNIT` | **8** | 4 – 15 | Radius รอบ player ที่กด R16 cross-faction acknowledge (ผ่านลาน / Wayfarer's Nod). ต่ำเกิน = ต้องเดินใกล้เกินไป ; สูงเกิน = trigger จาก stranger ที่ไม่ได้เห็นจริง | ✅ A/B กับ player feedback |
...
| TR-WMS-037 (R12 PlayFab failure rollback) | Logic | **Given** Azure Function teleport handler executing the R12 4-field `UpdateUserData` call ; mock PlayFab returns **HTTP 500 total failure** (NOT "partial-success" — PlayFab API does not emit partial-success per Cluster 2 #4 fix) **When** handler receives 500 **Then** transaction rollback fires: (a) ไม่มี Player Record field ใดถูก write (`UpdateUserData` is single server-side op — failure means no field changed) ; (b) instance population CAS reservation บน Redis ถูก release (`inst-pop:*` Lua script rollback) ; (c) client เห็น error toast retry-suggestion ; (d) state ยังคง pre-call value (`InCity` ที่ origin city ยัง valid) |
| TR-WMS-038a (R12 CAS hard ceiling — both reject) | Logic | **Given** instance_X pop=170, `hard_ceiling`=180 ; party A (size 12) และ party B (size 12) ส่ง co-location CAS request พร้อมกัน (race ใน Redis Lua script) **When** D1 step 1 evaluate ทั้งคู่ **Then** ทั้งคู่ reject — Lua script atomic check `170 + 12 = 182 > 180` fails for both regardless of ordering ; pop ยังคง 170 ; ทั้งคู่ fall through to step 2 (pack) ; emit `party_co_location_ceiling_exceeded` signal (party_id, instance_id) สำหรับทั้งคู่ |
| TR-WMS-038b (R12 CAS hard ceiling — sequential success) | Logic | **Given** instance_X pop=156, `hard_ceiling`=180 ; party A (size 12) และ party B (size 12) ส่ง co-location CAS request พร้อมกัน **When** Lua script serializes (ordering arbitrary) **Then** party หนึ่ง CAS success: read 156, claim 12, write 168 ; party ที่สอง retry: read 168 (Redis sub-ms makes retry deterministic), check `168 + 12 = 180 ≤ 180`, claim 12, write 180 ; **ห้าม end state เกิน 180 ในทุก scheduler ordering** ; ทั้งคู่ assigned instance_X |
| TR-WMS-053 (R12 read-repair null-instance fallthrough) | Logic | **Given** party member B's `current_instance_id` = "city_solis_03" บน Player Record ; mock LookupInstance returns null (instance city_solis_03 ถูก torn down per EC-04 ก่อน read) ; party A travel มา city_solis **When** D1 step 1 evaluate co-location candidate B **Then** B candidate ignored (treat as null per R12 read-repair contract) ; D1 falls through to step 2 (pack) ; ไม่มี error propagated to client ; ไม่มี exception thrown ; emit telemetry `read_repair_null_instance(player_id, city_id)` สำหรับ ops investigation |
| TR-WMS-054 (R12 cache TTL boundary) | Logic | **Given** Redis cache holds `cache:inst-pop:city_X:N=149` at t=0 with TTL=5s ; mock clock advance **When** read at t=4.9s **Then** cache hit, value=149, no PlayFab/Redis-authoritative-store roundtrip ; **When** read at t=5.1s **Then** cache miss → re-read from `inst-pop:*` authoritative store (Redis CAS keyspace) ; populated cache value reflects current pop (might be != 149 if writes occurred during cache window) |
...
| TR-WMS-027 (EC-11 ban while InCity, Keeper-voiced) | **Integration** | **Given** player state = `InCity` **When** M11 ส่ง `player_banned(player_id)` **Then** state transition → `OutOfUniverse` ภายใน 1s ; player ถูก force-exit จาก instance roster ; `current_population` ของ instance ลดลง 1 ; broadcast message ใน instance ตรงกับ Keeper-voiced phrasing *"ผู้ดูแลลานได้ขอให้ผู้ถือธงท่านหนึ่งออกจากลาน"* (Philosophy B per narr-2) ; **ไม่ระบุชื่อผู้ถูก ban** ; ไม่มีคำว่า "banned" หรือเหตุผล ; broadcast scope = instance only (R10) ; **When** ผู้เล่นเดียวกัน Galaxy Map travel ออก (voluntary) **Then** broadcast แทนเป็น *"[ชื่อ] ออกเดินทางจากลานแล้ว"* (ระบุชื่อ — แตกต่างจาก ban) ; **When** network drop (disconnect) **Then** broadcast = *"[ชื่อ] หลุดจากลานชั่วคราว"* — ทั้ง 3 cases distinct phrasing |
...
| TR-ID | Type | Given-When-Then |
|---|---|---|
| TR-WMS-034a (EC-19 prewarm cohort, launch-mode) | **Performance** | **Given** staging ตาม `production/qa/environment-spec.md` ; `LAUNCH_MODE_PREWARM_COUNT`=35 active ; `CITY_INSTANCE_SOFT_CAP`=150 ; load harness = single-threaded request injector (deterministic ordering) **When** inject 5,000 TravelRequest ไปยัง `STARTER_CITY_ID` ใน 10s window **Then** prewarm cohort (first ~5100 effective per EC-19 ghost-slot accounting): D1 step-3 invoke rate = 0% ; p95 latency ≤ 200ms ; ไม่มี EC-03 toast |
| TR-WMS-034b (EC-19 overflow cohort, steady-state) | **Performance** | **Given** staging ตาม env-spec ; `STARTER_CITY_INSTANCE_PREWARM_COUNT`=5 (steady-state, NOT launch-mode) ; load harness same as 034a **When** inject 5,000 TravelRequest ใน 10s **Then** overflow cohort (751–5000): D1 step-3 invoke rate ≥ 80% (acknowledged degraded mode) ; p95 latency ≤ 2000ms ; EC-03 toast tolerated สำหรับ ผู้เล่น ที่ hit SpawnNewInstance fail ; ไม่มี HTTP 5xx ; total spike resolution ≤ 60s post-spike-end |
| TR-WMS-034c (PlayFab rate-limit ceiling) | **Performance** | **Given** staging ตาม env-spec ; PlayFab CCU plan tier ระบุใน env-spec.md (default "Indie Studio" 2500 calls/s) **When** TR-034a หรือ TR-034b รัน **Then** measured PlayFab call rate ≤ contracted CCU plan ceiling × 0.95 (5% headroom) ; ถ้าเกิน → fail + log "rate-limit-exceeded" reason ; verify D1 happy-path call cost ≤ 3 calls/req per R12 budget |
...
| TR-WMS-042 (EC-19 mid-range overflow 751–999) | **Performance** | **Given** staging ตาม env-spec ; pre-warm=5 (capacity 750 raw / ~600 effective per EC-19 ghost-slot accounting) **When** inject 900 TravelRequest ไปยัง `STARTER_CITY_ID` ใน 10s window **Then** ผู้เล่น 601–900 hit D1 step 3 SpawnNewInstance fallback ; p95 latency สำหรับ overflow cohort ≤ 2000ms (acknowledged spike per pre-warm boundary) ; ไม่มี HTTP 5xx |
| TR-WMS-055 (EC-14 ghost-slot accounting under spike) | **Performance** | **Given** staging ตาม env-spec ; `STARTER_CITY_INSTANCE_PREWARM_COUNT`=5, `ENTERING_HOLD_SECONDS`=30 ; mock network layer drop 10% ของ EnteringCity sessions **When** inject 5000 TravelRequest ใน 10s window + hold for 30s post-spike **Then** measured peak ghost-slot count ≤ predicted by EC-19 formula (`peak ≈ disconnect_rate × spike_size × ENTERING_HOLD/spike_window` = 150 ± 10%) ; effective prewarm capacity ≈ 600 (within 10% of formula prediction 750 − 150) ; ghost slots fully released by t=spike_end + ENTERING_HOLD + 5s tolerance |
...
### OQ-2 — Theme เฉพาะตัวของ 10 cities (deliverable spec partial — closes narr-3)
- **Owner:** **Narrative Director (lead) + World Builder + Art Director (co-own)**
- **Status:** **Theme spec template + 2 cities (city_01 STARTER + city_solis)
  authored 2026-05-05 narrative pass. Remaining 8 cities pending narrative
  writer-day allocation per producer notification below.**
- **Trigger:** Pre-production narrative pass continues (8 cities) →
  art bible faction archetype section → CBS theme spec lock
...
- **Cross-ref:** Cascade-blocks OQ-3 (Anchor list) และ vignette authoring (R3.1)

#### OQ-2 Theme Spec Template (per-city brief format)

ทุก city brief ต้องมี 6 fields เหล่านี้:

```markdown
## City Theme Brief — [city_id]
**city_id:** [e.g., city_01, city_solis]
**Provisional name:** [ชื่อใน GDD + walkthroughs]

### Cultural register (1-2 sentences)
ลักษณะวัฒนธรรม — tradition, กิจกรรมหลัก, บรรยากาศสังคม.
ห้ามกล่าวถึง faction ใดโดยตรง (R1 neutrality).

### Anchor Content type
หนึ่งใน: chronicle-of-the-war / artifact-chamber / observatory /
lore-archive / cosmetic-preview-theatre / [shared-witness type —
≥2 ของ 10 เมืองต้องเป็น shared-witness/relational เพื่อแก้
narr-2026-04-30 nice-to-have R3 solo-consumption escalation]

### Bell instrument designation
"[primary] + [secondary]" สำหรับ R8 audio.

### First-visit environmental beat text (Thai)
1-2 ประโยค atmospheric เท่านั้น — ห้ามอธิบาย service/UI/wayfinding
(Section B lines 91-93 binding).

### Banner Rack visual treatment note (1 sentence)
ทิศทาง art สำหรับ R7 + EC-23 ; plurality design (≥1 per faction
present per narr-6).

### Ambient soundscape brief (1 sentence)
ทิศทาง audio direction.
```

#### city_01 (STARTER_CITY_ID — provisional) full brief

- **Cultural register:** เมืองที่ตั้งอยู่บนจุดตัดทางเส้นทางเก่า — ผู้คน
  มาจากทิศทางต่างๆ โดยไม่มีทิศทางใดเป็น "ทิศหลัก" ; ธรรมเนียมของที่นี่
  คือการต้อนรับผู้มาเยือนครั้งแรกโดยให้พวกเขาค้นพบลานด้วยตัวเอง ไม่ใช่
  การนำทาง
- **Anchor Content type:** **lore-archive** — บันทึกเส้นทางของผู้เดินทาง
  หน้าใหม่จากทุกยุค (pre-faction-conflict era เป็นหลัก) ; ไม่มีการบันทึก
  ที่ศูนย์กลาง faction ใด
- **Bell instrument:** **"bronze hand-bell + single sustained string"** —
  เสียงเรียบง่าย, ไม่ประดับประดา, บ่งชี้ "เริ่มต้น" ไม่ใช่ "ยิ่งใหญ่"
- **First-visit beat text:** *"ลานแห่งนี้ไม่มีประตูกั้นฝ่าย — ระฆังจะ
  บอกให้รู้เองว่าอะไรกำลังจะมา"*
- **Banner Rack visual:** plurality design — display ≥1 banner ต่อ faction
  ที่มีใน instance, ไม่ใช้ proportional sizing (ที่ STARTER_CITY ผู้เล่นใหม่
  มาจากหลาย faction, proportional อาจทำให้ rack ดูเหมือน "เมืองของฝ่าย X")
- **Ambient soundscape:** sparse-outdoor — ลม + footsteps ห่างๆ + string
  texture เบา ; ไม่มี percussion หนัก ; "มาถึงที่เงียบสงบหลังเดินทาง"

#### city_solis full brief

- **Cultural register:** เมืองที่มีประเพณีการจดบันทึกท้องฟ้า — ชุมชนที่
  tracking ดาว, ฤดูกาล, และเหตุการณ์ใหญ่ผ่านการสังเกตการณ์ ; ธรรมเนียม
  คือ "ดูก่อน ตัดสินทีหลัง" ทำให้ลานนี้รู้สึกช้าลงกว่าเมืองอื่น
- **Anchor Content type:** **observatory (shared-witness type)** — กล้อง
  โทรทรรศน์กลาง plaza เปิดให้ผู้เล่นใช้ร่วมกันได้ (cooperative-use:
  player 2 คนขึ้นไปใน proximity trigger ภาพท้องฟ้าที่ละเอียดกว่า 1 คน
  เล็งคนเดียว). **ไม่มี reward, ไม่มี XP** — relational moment ที่ R3
  อนุญาต ; cooperative behavior ใช้ R12/R13 presence aggregator ที่มี
  อยู่แล้ว, ไม่ใช่ mechanic ใหม่
- **Bell instrument:** **"singing bowl + sustained glass harmonica"** —
  เสียงยืนยาว, ก้องช้า ; เหมาะกับเมืองที่ชีวิตดำเนินตามจังหวะของท้องฟ้า
- **First-visit beat text:** *"ที่นี่คนนับดาวก่อนนับเวลา — ลานเปิดรับ
  ทุกคนที่รู้จักการรอ"*
- **Banner Rack visual:** circular-arc layout รอบ plaza center (ไม่ใช่
  linear wall) เพื่อ reflect ธรรมเนียม "มองรอบทิศ" ของเมือง ; plurality
  design เหมือนกัน
- **Ambient soundscape:** night-observatory — wind ต่ำ + singing bowl
  decay ยาว + occasional distant bell texture ; ไม่มี crowd noise หนัก ;
  "มองขึ้นไปแล้วเงียบ"

#### Producer notification (2026-05-05) — 8 cities pending

city_01 และ city_solis ได้รับ theme brief สมบูรณ์ใน narrative pass 2026-05-05.
**เมืองที่เหลืออีก 8 เมืองต้องการ narrative writer pass ระดับ theme spec
ก่อนที่ audio direction, art direction, asset spec, และ vignette pool
authoring จะเริ่มได้** — OQ-2 เป็น upstream dependency ของทั้งสี่
workstream นั้น.

ประมาณการ: theme brief ต่อเมือง ≈ 0.5 narrative writer-day (รวม cultural
register, bell instrument, ambient brief, beat text) × 8 เมือง = **4
narrative writer-days สำหรับ theme spec** (แยกจาก 80 wd ของ vignette
pool ที่ประมาณไว้ใน OQ-3 producer note ด้านล่าง).

แนะนำ lock OQ-2 theme spec ทั้ง 10 เมือง **ก่อน sprint commit ของ audio
direction pass** — ไม่เช่นนั้น audio team จะ author bell instrument และ
soundscape บนสมมติฐานชั่วคราวที่อาจต้องแก้ไขซ้ำ.

**Dependency chain:** OQ-2 theme spec → OQ-3 Anchor list → vignette pool
authoring + audio direction + asset spec + first-visit beat text per city.
