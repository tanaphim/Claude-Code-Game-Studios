...
> **Status**: In Design (revised post second review 2026-04-29)
> **Author**: User + Claude Code agents
> **Last Updated**: 2026-04-29
> **System ID**: FT12
> **Revisions**:
> - 2026-04-28 — addressed 8 blocking items from /design-review #1 (D1 ceiling+freshness fix, R12 atomicity, G.5 fairness window, G.6 knob constraint, R3.1 vignette rotation, R15 ambient affordances, OQ-9 stranger interaction)
> - 2026-04-29 — addressed BLOCKING items from /design-review #2 with 6 specialists (R12 single-doc JSON blob model, D1 TOCTOU fix, Section B fantasy reframe, R16 cross-faction acknowledge primitive, R3 R1 violation scrub, EC-11 Thai phrasing fix, ANCHOR_VIGNETTE_POOL_MIN 4→8, HARD_CEILING_RATIO max 1.5→1.1, TR-WMS-034/035 fix, ACs added for R3.1/R15/R16 + atomicity rollback + EC-14/18/24, Accessibility baseline configured, OQ-2/3 converted to milestones, Galaxy Map interrupt rule, R13 counter relabel, EC-09 auto-blur, EC-26 sequencing, Keeper NPC brief)
...
นอกแมตช์ที่ผู้เล่นหลากฝ่ายเดินอยู่ร่วมกันใน 10 เมืองกลาง (ดาวเคราะห์)
...
ในทางผู้เล่น 10 Cities คือ **บ้านกลางที่ผู้เล่นหลากฝ่ายมาเจอกันโดยไม่
ต้องเป็นศัตรู** — สังกัดไม่กำหนดทางเดิน, การพบเพื่อนข้ามฝ่ายเป็นเรื่องปกติ,
และเมืองที่อยู่ตอนนั้น render badge/chat color ตาม `player.faction_id`
แต่ไม่กระทบสิทธิ์เข้าเมืองหรือ matchmaking pool.
**Sharding note:** ผู้เล่นในลานเดียวกันจริง ๆ คือ instance roster
(default ~150 คน, R13) — co-presence เป็น **per-instance** ไม่ใช่
city-wide. Fragment Event bell + Monument transform broadcast ข้าม
instance พร้อมกัน (R8) เป็น collective beat ที่ใช้ส่งต่อ "เสียงร่วมลาน"
แทนการ render avatar ข้าม instance (constraint จาก R10/R13). ในทางระบบ World Map
...
LoL/DotA, การมาถึงเมืองแรกครั้งแรกต้องทำงาน **สอง job แยกกัน**:

1. **Tone job (environmental beat)** — ระฆังเมืองดังต้อนรับ +
   ข้อความเงียบๆ (non-popup, fade in/out) ที่ตั้ง tone "ลานกลาง"
   ตั้งแต่วินาทีแรก. **Provisional string** *(writer pass required)*:
   *"ลานนี้เป็นดินแดนกลาง — ผู้ถือธงทุกฝ่ายแวะพักที่นี่เสมอกัน"*
2. **Functional job (orientation)** — ผู้เล่นต้องรู้ภายใน 30 วินาที
   ว่า: (a) Keeper NPC อยู่ที่ไหน + เปิด City Menu ได้, (b) Galaxy
   Map hotkey/UI button คืออะไร, (c) Fragment Event Monument คือ
   landmark ใด. **ห้าม collapse** ทั้งสอง job ไว้ใน beat เดียว —
   tone beat ทำงานก่อน, functional orientation ตามมาเป็น 2nd beat
   (delay 3–5 วินาที). UX spec รับ implementation; GDD commit ว่า
   ทั้งสอง job ต้องมี + tone ต้องมาก่อน functional.
...
astronomical archive, cartographer's record, fragment geological survey,
cosmetic preview theatre, lore archive) — รายการสมบูรณ์ [Open Question:
Anchor Content ของ 10 cities, owner = narrative team].
**R1 alignment rule:** Anchor Content ห้ามเป็น faction-coded content
(ห้าม "faction history hall" หรือใดๆ ที่ privilege faction หนึ่งเหนืออื่น)
— neutrality test: *"Anchor นี้มีความหมายเท่าเทียมกันสำหรับผู้เล่นที่
เพิ่งเปลี่ยนฝ่ายเมื่อวานหรือไม่?"* ถ้าตอบ "ไม่" = R1 violation.
...
**R5.1 — Dual-purpose interrupt rule**
ขณะ Galaxy Map เปิดอยู่และผู้เล่นกำลัง select destination: ถ้า Fragment
Event ใหม่ announce, indicator pulse ของเมืองนั้น update **passive**
(visual change บน Galaxy Map) — ห้าม interrupt selection flow, ห้าม
auto-redirect, ห้าม modal popup. ผู้เล่นต้อง complete หรือ cancel
travel selection ปัจจุบันก่อน, แล้วจึงตัดสินใจเรื่อง event ใหม่ได้.
*(travel intent = active task ; event announcement = ambient information)*
...
**Cross-instance broadcast bus = ADR-level decision** — R8 ระบุ behavior
"ทุก instance พร้อมกัน" แต่ implementation pattern (Photon AppServer relay
vs SignalR group fan-out vs PlayFab event messaging vs hybrid) **defer
to ADR** ร่วมกับ R13 sharding pattern. ADR ต้องระบุ broadcast latency
budget (TR-WMS-009 = 500ms server-side), connection lifecycle, และ
bandwidth ceiling สำหรับ 10 cities × N instances × M players.
...
**R9.1 — Cross-faction Fragment routing trade-off (accepted)**
การที่ผู้เล่นแข็ง form cross-faction party แล้ว Fragment route กลับ
แต่ละ faction ตัวเอง (FT11 R4) อาจสร้าง perverse incentive: ผู้เล่น
แข็งกระจาย skill ข้าม faction → ทำลาย concentration ของ Fragment pool
ฝั่งตัวเอง. **Position:** ยอมรับ trade-off นี้ — design ไม่ใส่ counter
mechanic. Rationale: P3 Team Synergy (cross-faction friendship) มี
priority สูงกว่า P4 faction-pool optimization ในระดับ supporting pillar
ranking ; ผู้เล่นที่ value faction-loyalty ยังคงเลือก solo/same-faction
queue ได้. Telemetry hook: FT12 expose `cross_faction_party_match_pct`
metric ; ถ้า >70% ของ matches มี cross-faction premade → re-evaluate
trade-off ในรอบ live ops review.
...
**Fantasy gap acknowledgment:** R10 + R13 sharding หมายความว่า "ทุกฝ่าย
มาเจอกัน" ของ Section B ทำงานในระดับ **per-instance** (≤150 คน, AOI
~30–40 คนใน chat radius) ไม่ใช่ city-wide. Cross-instance collective
awareness = bell + Monument transform เท่านั้น (R8). ระบบไม่ได้พยายาม
แก้ proximity-chat scope ให้กลายเป็น city-wide — เป็น principle-driven
choice (P5 anti-toxicity > scale fantasy).
...
**R12 — Server Authority + Single-Document Player Record**
...
**Atomicity model — single-document JSON blob:**
ทั้ง 4 fields ถูกเก็บภายใน **PlayFab Entity Object key เดียว** ชื่อ
`ft12_player_state` (one-document write). Azure Function สร้าง JSON
blob ใหม่จาก current state + delta แล้วเขียนทับด้วย `SetObjects` call
เดียว — PlayFab รับประกัน atomicity ระดับ document (single-key replace
= all-or-nothing). ไม่มี multi-key transaction ; ไม่มี partial-commit
state เป็นไปได้ในทาง infrastructure.

**Constraint จาก single-document model:**
- **อ่านพร้อมกันทั้ง 4 fields เสมอ** — ห้าม consumer อ่านแค่ field
  เดียวจาก blob (ทุก read = full document fetch + parse)
- **Write conflict resolution:** ใช้ PlayFab `IfChangedFromDataVersion`
  conditional write ; ถ้า version mismatch (race ระหว่าง 2 concurrent
  writers) → reject 2nd writer + retry by re-reading current state
- **Idempotency:** Azure Function ใส่ `idempotency_key = hash(player_id,
  client_request_id)` ใน blob ; replay ของ same request → no-op
- **Field set ภายใน blob (canonical schema):**
  ```json
  {
    "last_city_id": "city_astra",
    "last_city_entry_timestamp": "2026-04-29T12:00:00Z",
    "current_instance_id": "city_astra_01",
    "ft12_migrated": true,
    "version": 42,
...
โดยไม่ lookup. *(ป้องกัน dangling reference จาก crash ระหว่าง write
หรือ race ระหว่าง EC-04 teardown กับ co-location read — read-repair
เป็น compensating action ไม่ใช่ rollback)*

**Instance population atomicity (separate from Player Record):**
`instance.current_population` เก็บใน Instance Record (ไม่ใช่ Player
Record blob). D1 step 1 ceiling check ต้องใช้ **atomic
increment-with-ceiling** primitive (server-side check-and-set):

```
TryIncrementPopulation(instance_id, delta, ceiling) → bool:
  // atomic: read current, check (current + delta) ≤ ceiling, write back
  // implementation: PlayFab Entity Object IfChangedFromDataVersion
  //   หรือ Azure Table OData merge with ETag
  return success / failure (race lost or ceiling exceeded)
```

D1 step 1 ที่เคย "read pop → check ceiling → write" (TOCTOU race) ถูก
แทนด้วย "TryIncrementPopulation(target, party_size, hard_ceiling)" ; ถ้า
return false → emit_signal + fall through ไป step 2.
...
City_Astra_02, …).
- Players เห็น **cross-instance counter** ที่ plaza entrance: *"ลานนี้ X คน / เมืองนี้ทั้งหมด Y คน"* — relabeled (was "ลานร่วม") เพื่อกัน false joinable affordance ; counter เป็น informational ไม่มี join action
...
ทุก plaza ต้องมี **non-mechanical social affordances อย่างน้อย 5 ประเภท**
...
4. **Ambient gathering hooks (art/level direction)** — จุดที่ NPC +
   player co-mingle ใน formation ที่อ่านเป็น "ลานชุมนุม" จาก camera
   angle ปกติ. *(scene composition ; defer ไป Asset Spec — ไม่ใช่
   player action)*
5. **Monument ambient countdown (ongoing watchable)** — Monument แสดง
   passive ambient state ตลอดเวลาแม้ไม่มี Fragment Event active:
   particle/glow ระดับต่ำที่ pulse คล้ายลมหายใจ ; ในช่วง 60 วินาที
   ก่อน Fragment Event announce อาจถัดเข้าระดับสูงขึ้นเล็กน้อย
   (anticipatory beat). ไม่ใช่ countdown ตัวเลข, ไม่ใช่ HUD element ;
   เป็น ambient visual ที่ผู้เล่นในลานเห็นพร้อมกัน — **ongoing
   activity ที่แข่งกับ City Menu pull**. ไม่ require interaction.
...
ผ่าน chat หรือ party invite — เพื่อส่งมอบ Section B fantasy "ทุกฝ่ายมา
เจอกันโดยไม่ต้องเป็นศัตรู" ในระดับ encounter ไม่ใช่แค่ presence.

**Design intent (GDD-level commitment ; UX spec รับ implementation):**
- **1-button targeted acknowledge action** (working name: "ผ่านลาน /
  Traveler's Salute") — กดที่ player target ใน radius `R16_ACKNOWLEDGE_RADIUS_UNIT`
  (default 8, ดู Section G) → trigger reciprocal salute emote บนทั้งสอง
  ฝั่ง พร้อมกัน
- **No mechanical reward** (R3 rule extends ที่ R16) — ห้าม XP, Fragment,
  reputation, currency, queue priority ใดๆ
...
- **Rate limit:** 1 acknowledge ต่อ player target ต่อ 60 วินาที
  (กัน spam ; ระดับ R10 toxicity surface)
- **Opt-out:** ผู้เล่นที่ปิด emote receive ใน settings ไม่เห็น
  reciprocal animation จาก stranger (M11 boundary)

**Implementation defer to UX spec:** กdการ trigger UI (long-press vs
click-target vs hotkey), animation spec, audio cue, accessibility
(non-mouse input model) อยู่ใน UX spec phase. R16 GDD commit แค่
**design contract**: primitive ต้องมี + ห้ามมี mechanical reward +
ต้อง cross-faction visually distinct.

*(Rationale: Section B ของ GDD นี้ถูก review เป็น fantasy-load-bearing
โดย game-designer + ux-designer ในรอบ /design-review #2 ; OQ-9 จาก
รอบแรกถูก convert เป็น R16 commit เพราะ defer ไป UX spec ทำลาย Section
B fantasy contract)*
...
InstanceAssignment(player, city_id, t):
  ...
  // R12 fix: ceiling check + population write = atomic via TryIncrementPopulation
  //   (prevents TOCTOU race when 3 parties co-locate-race the same target)
  for each member in party:
    if member.current_instance_city_id == city_id
      ...
      AND member.last_heartbeat_age_seconds < 5:         // EC-07 freshness window (EXCLUSIVE at 5.0)
      target = LookupInstance(member.current_instance_id, t)
      if target is null:
        continue                                         // stale ref — instance torn down
      // EC-06 hard ceiling enforcement — atomic check-and-set (R12)
      // Replaces previous "read pop → check → write" TOCTOU pattern
      if TryIncrementPopulation(target.instance_id, party_size, hard_ceiling):
  ...
  // R12 fix: same atomic increment-with-ceiling pattern as step 1
  ...
  for each i in candidates:
    if TryIncrementPopulation(i.instance_id, party_size, CITY_INSTANCE_SOFT_CAP):
      return i.instance_id                               // assign (race-safe)
...
- **EC-09** **If match-found countdown trigger ขณะ player พิมพ์ใน text input
  (loadout name, friend search)**: ระบบ **auto-blur input + auto-save
  current text** ทันทีที่ match-found signal มาถึง, แล้วแสดง countdown
  modal ตาม priority hierarchy (priority 1, interrupt ทุกอย่าง). ผู้เล่น
  ไม่เสีย typed text ; Enter key conflict ถูกแก้โดย focus shift ไม่ใช่
  defer. *(แก้ contradiction กับ priority hierarchy ที่เคยมี — ux-designer
  /design-review #2 finding F2)*
...
- **EC-11** **If ban event มาถึงขณะ player `InCity`**: state transition →
  ...
  *"[Player] ได้ออกจากลาน [City Name] แล้ว"* (ไม่ระบุเหตุผลเพื่อรักษา
  dignity). เมื่อ ban-lifted: login ครั้งถัดไป spawn `last_city_id`
  ปกติ (R4). *(Thai phrasing fix /design-review #2 — original "โดยกำเนิด"
  = "by birth" ผิด context)*
...
- **EC-26** **If Fragment Event Monument อยู่นอก viewport ขณะ announce**:
  ...
  (UX spec รับ implementation):** (1) bell audio fires ที่ t=0 ;
  (2) directional on-screen indicator fade-in ที่ t=+0.3s ; (3) Galaxy
  Map pulse เป็น passive ambient (ไม่ trigger UI sound). กัน sensory
  stack pressure ที่ frame 0.
...
| `CITY_INSTANCE_HARD_CEILING_RATIO` | **1.1** (→165) | 1.0 – 1.1 | Multiplier ของ soft cap สำหรับ R9 party co-location override. กันไม่ให้ instance เดียวรองรับ party หลายชุดจน balloon. **Lowered จาก 1.2/1.5 → 1.1/1.1 max** /design-review #2 finding C: ที่ 1.2 → 180 player ใน RadiusChat 15 = 40–50 คน ละเมิด G.1 "tolerable ~30–40" band | ❌ Principle-driven |
...
| `ANCHOR_VIGNETTE_POOL_MIN` | **8** | 8 – 16 | จำนวน vignette ขั้นต่ำต่อ Anchor (R3.1). **Raised จาก 4 → 8** /design-review #2 finding N3: pool=4 × rotation=7d = exhaustion ใน 4 สัปดาห์ ทำให้ R3.1 fail purpose ตัวเอง. Floor 8 รองรับ 8 สัปดาห์ก่อน repeat = ~2 เดือน. Launch authoring cost = 80 vignettes (10 cities × 8) | ❌ Content-driven |
...
| Knob | Default | Safe Range | What it affects | Live-tune? |
|---|---|---|---|---|
| `CITY_VISIT_FAIRNESS_INDEX_ALERT` | **0.3** (gini-style) | 0.1 – 0.7 | Threshold ของความไม่เท่ากันของ city traffic distribution (EC-21). **Computation:** Gini coefficient ของ `unique_visitors_per_city` (10 cities — รวมเมืองที่ visits=0) ; sample = rolling 24h window ; aggregate = daily snapshot at 00:00 UTC. **Sensitivity note:** Gini บน 10 buckets ที่มี zero-bucket จะดึงค่าขึ้นทาง 1 ; 0-visit city เพียงวันเดียวอาจ inflate metric ; trigger ต้องการ 7 consecutive daily samples ≥ threshold เพื่อ dampen noise. **Trigger:** 7 consecutive daily samples ≥ threshold ⇒ "Anchor concentration warning" → re-engagement campaign / event redistribution review | ✅ Live-tune |
...
| `CITY_INSTANCE_FRAGMENTATION_ALERT` | **0.40** (40% half-empty) | 0.20 – 0.80 | Fraction ของ active instances ที่มี `current_population < SOFT_CAP × 0.5` ใน rolling 1h window. > threshold ⇒ first-fit-descending packing ทิ้ง runt instances สะสมจน server resource เปลือง ; ops ตัดสินใจ defrag (drain instance ที่ pop ต่ำที่สุด → migrate players → teardown). /design-review #2 finding B (item 9 prior log) | ✅ Live-tune |
| `CROSS_FACTION_PARTY_MATCH_PCT_ALERT` | **0.70** (70%) | 0.30 – 0.90 | Fraction ของ Tournament matches ที่มี cross-faction premade ≥ 1 party. > threshold ⇒ R9.1 trade-off ผลลัพธ์เกินคาด — re-evaluate cross-faction party cap. /design-review #2 finding I1 | ✅ Live-tune |
...
- `CITY_INSTANCE_HARD_CEILING_RATIO` × ขนาด party สูงสุด (M1 owns) →
  ...
  ไม่เกิน CITY_INSTANCE_SOFT_CAP × 2. **PARTY_MAX placeholder = 5**
  (M1 placeholder ; ต้อง confirm ใน entity registry — /design-review #2
  finding H). At PARTY_MAX=5, HARD_CEILING_RATIO=1.1, SOFT_CAP=150 →
  ceiling=165 รับได้ทุก scenario
- `ANCHOR_VIGNETTE_POOL_MIN` × `ANCHOR_VIGNETTE_ROTATION_DAYS` →
  content lifetime ก่อน repeat exposure. Min lifetime = pool × rotation
  ≥ 28 days (constraint) ; ที่ default (8 × 7 = 56 days) ปลอดภัย ;
  ที่ floor pool=8, rotation=1 → 8 days ละเมิด constraint, CBS validator
  reject
...
- **Keeper NPC archetype (1 per city, narrative-coded)** — *guardian of
  the neutrality pact ; greets travelers, mediates City Menu access,
  speaks with the "ผู้ดูแลวิหาร" tone (สุภาพแต่หนักแน่น)*. Each city's
  Keeper shares this archetype but has theme-specific name/visual
  (defer to OQ-2/OQ-3 narrative milestone). Provisional first-line:
  *"คุณกำลังเข้าสู่ลานเมือง [City Name] — ผู้ถือธงทุกฝ่ายร่วมแวะพักที่นี่"*
  *(provisional — writer pass required)*
...
| **Cross-instance counter** | R13 | "ลานนี้ X คน / เมืองนี้ทั้งหมด Y คน" ที่ plaza entrance — informational only, no join action |
...
**Baseline commitments (GDD-level minimum ; UX spec รับ detailed
implementation):**

- **Target platform (assumed default per OQ-7):** PC + keyboard/mouse —
  **primary input model สำหรับ FT12** จนกว่า OQ-7 จะ resolve. Console
  (gamepad) + mobile (touch) = TBD ; ถ้า leadership commit เพิ่ม,
  Galaxy Map UI, Keeper interaction, R16 acknowledge primitive ต้อง
  re-validate ใน UX spec
- **Color accessibility (Fragment Event indicators, faction badges):**
  Fragment Event state (active vs dormant) **ห้าม encode ด้วยสีเพียง
  อย่างเดียว** — ต้องมี shape/icon/animation cue ที่ distinguishable
  สำหรับ deuteranopia/protanopia. Faction badges ต้องผ่าน colorblind
  contrast check (defer specific palette ไป Art Bible)
- **Text scaling:** UI text ต้องรองรับ 100% / 125% / 150% scale ขั้น
  ต่ำ (3 steps) ; first-visit beat text ต้อง wrap ที่ 150% scale
  โดยไม่เกิน viewport
- **Audio cues require visual fallback:** ระฆัง Fragment Event (R8)
  ต้องมี on-screen indicator แม้ audio mute หรือ deaf player (EC-26
  directional indicator + Galaxy Map pulse ทำหน้าที่นี้แล้ว — ห้ามถอด)
- **Input remapping:** Galaxy Map hotkey, R16 acknowledge button ต้อง
  remappable (ไม่ hardcode)
- **Screen reader / motor accessibility:** defer to UX spec phase แต่
  GDD ห้าม block — ห้ามใส่ time-pressure interaction ที่บังคับ rapid
  input ใน plaza (R15 affordances opt-in, R16 single-button, EC-09
  modal countdown มี ≥ 5s window)

**Open items (defer to UX spec, but not blocking GDD approval):**
- Specific WCAG conformance level (AA vs AAA)
- Subtitle/caption system สำหรับ ambient audio
- Motion sickness mitigation สำหรับ Galaxy Map transition

> **Cross-ref:** OQ-7 (platform/input) ส่งผลต่อ baseline นี้ ; ถ้า
> leadership ระบุ console/mobile, UX spec phase ต้อง re-validate
> 17 UI surfaces.
...
>
> **Coverage:** 47 criteria — Core Rules 15 + R3.1/R15/R16 = 18,
> Formulas 6, Edge Cases 12 (added EC-14/18/24 + atomicity rollback),
...
| TR-WMS-006 (R5) | **Integration** | **Given** player state = `InCity`, `GALAXY_MAP_TRANSITION_SECONDS` = 2.5 **When** player คลิก city ปลายทาง → confirm บน Galaxy Map overlay **Then** server-side `EnteringCity` state ตั้งขึ้นภายใน 200ms (server timestamp delta) ; scene load complete + `InCity` state ที่ปลายทางใน ≤ `GALAXY_MAP_TRANSITION_SECONDS` (2.5s server-side) ; client scene load ขึ้นกับ `ENTERING_TIMEOUT_SECONDS` (30s SLA, separate). Test ทุกอย่างวัด server-side timestamps เท่านั้น ; ไม่วัด client render latency |
...
| TR-WMS-009 (R8) | **Integration** | **Given** mock FT14 ส่ง `fragment_event_started(city_id="city_nova", event_id="evt_001")` (minimal mock schema — pending OQ-4) **When** FT12 รับ event ที่ FT12 Azure Function entry point (server-side timestamp T0) **Then** ภายใน 500ms จาก T0 (measured at last instance broadcast confirmation): (1) Monument ของ city_nova **ทุก active instance ที่ registered ที่ T0** emit `transform_active` signal ; (2) Galaxy Map indicator ของ city_nova = `active` ; (3) audio event `city_bell_ring` trigger ; FT12 ไม่เขียน Fragment state ใดๆ. **Note:** AC valid against minimal mock เท่านั้น ; ต้อง revise เมื่อ OQ-4 (FT14 schema) resolve. Cross-region propagation latency excluded จาก SLA |
...
| TR-WMS-027 (EC-11 ban while InCity) | **Integration** | **Given** player state = `InCity` **When** M11 ส่ง `player_banned(player_id)` ที่ FT12 handler (server timestamp T0) **Then** state transition → `OutOfUniverse` ภายใน 1s จาก T0 (server-side delta) ; player ถูก force-exit จาก instance roster ; `current_population` ของ instance ลดลง 1 ; broadcast message ใน instance = "[Player] ได้ออกจากลาน [City Name] แล้ว" — ไม่มีคำว่า "banned" / ไม่ระบุเหตุผล |
...
| TR-WMS-029 (EC-23 faction switch live) | **Integration** | **Given** player A อยู่ใน city_astra_01 ร่วมกับ player B **When** A complete faction switch (FT11 R6) ขณะ `InCity` ; FT12 ได้รับ presence-delta event ที่ T0 **Then** instance_channel broadcast ภายใน 2s จาก T0 (server-side ; client render latency excluded) ; chat messages เก่าของ A ใน instance ยัง render ด้วย color faction เดิม (immutable post-send) |
...
| TR-ID | Type | Given-When-Then |
|---|---|---|
| TR-WMS-034 (EC-19 launch throughput) | **Performance** | **Given** staging ตั้ง `STARTER_CITY_INSTANCE_PREWARM_COUNT`=**34** (raised from 5 — math fix /design-review #2 finding D: 34 × 150 = 5,100 ≥ 5,000), `CITY_INSTANCE_SOFT_CAP`=150 **When** load test inject 5,000 TravelRequest ไปยัง `STARTER_CITY_ID` ใน 10s window **Then** p95 instance assignment latency ≤ 500ms ; D1 step 3 SpawnNewInstance invoke rate < 10% ของ requests (≤ 500/5,000) ; ไม่มี HTTP 5xx. **Note:** Default `PREWARM_COUNT`=5 รองรับ 750 concurrent — launch playbook bump เป็น 34 เฉพาะ launch day แล้ว revert (cross-ref G.6 launch policy) |
| TR-WMS-034b (EC-19 boundary 750→1000) | **Performance** | **Given** `STARTER_CITY_INSTANCE_PREWARM_COUNT`=5, `CITY_INSTANCE_SOFT_CAP`=150 (default config) **When** load test inject 1,000 TravelRequest ใน 10s (overflow 750 prewarm capacity by 250) **Then** 750 players ได้ instance ภายใน p95 ≤ 200ms ; remaining 250 players trigger D1 step 3 → assigned ภายใน p95 ≤ 1.5s ; spawn-failure toast (EC-03) rate < 1% ; ไม่มี cascading retry storm. /design-review #2 finding D (item 11 prior log) |
| TR-WMS-035 (EC-20 Fragment Event spike) | **Performance** **(BLOCKING — reclassified from ADVISORY /design-review #2 finding D9: R8 'พร้อมกัน' contract ทำให้ TR นี้เป็น hard release gate)** | **Given** `EVENT_ANNOUNCE_LEAD_SECONDS`=30, `D2_SCHEDULER_TICK_SECONDS`=5 (default — note tick=6 ที่ขอบ safe range อาจ fail timing assertion ; CBS ห้าม set tick > 5 ในบริบท TR-WMS-035) **When** Fragment Event announce trigger แล้ว 1,000 TravelRequest ไปยัง event city ใน 5s หลัง announce **Then** D2 pre-warm trigger ≥ 1 ครั้งภายใน 5s แรกหลัง announce (boundary inclusive at t=5.0s) ; p95 assignment latency ≤ 500ms ; ไม่มี thundering-herd SpawnNewInstance burst > 3 instance ต่อ tick |

### H.5b Added ACs from /design-review #2 (BLOCKING gap fixes)

| TR-ID | Type | Given-When-Then |
|---|---|---|
| TR-WMS-036 (R3.1 vignette rotation) | **Integration** | **Given** city_astra Anchor มี vignette pool ขนาด 8, `ANCHOR_VIGNETTE_ROTATION_DAYS`=7, current server UTC = Monday 00:00 **When** test fixture advance server clock 7 days **Then** Anchor.current_vignette_index เปลี่ยนเป็น `(prior_index + 1) mod 8` ; vignette content แสดงขณะ player interact = vignette ลำดับใหม่ ; rotation event log ที่ Monday 00:00 next week |
| TR-WMS-037 (R3.1 server-clock atomic) | **Integration** | **Given** 3 instances ของ city_astra (Astra_01/02/03) **When** rotation event trigger ที่ Monday 00:00 UTC **Then** ทุก 3 instances แสดง vignette index เดียวกันภายใน 5s ของ rotation event (server-clock authoritative ; per-instance state ไม่ drift) |
| TR-WMS-038 (R3.1 pool floor enforcement) | Logic | **Given** narrative team ส่ง vignette pool < 8 ผ่าน CBS ingestion **When** CBS validation run **Then** validation reject + error message *"Anchor vignette pool must contain ≥ ANCHOR_VIGNETTE_POOL_MIN entries"* ; CBS ไม่ commit config |
| TR-WMS-039 (R15 no-mechanical-reward) | Logic | **Given** player interact กับ R15 affordances ทุก 5 ประเภท (bench, emote spot, fountain, gathering hook, Monument ambient) **When** interaction complete **Then** Player Record delta = ∅ (zero changes ใน XP, Fragment, currency, hero stat, queue priority, faction pool) |
| TR-WMS-040 (R15 no-traversal-block) | Logic | **Given** automated agent ทดลองเดินผ่าน plaza จาก entrance ถึง center plaza บน 10 cities **When** path traversal run **Then** ไม่มี R15 affordance object block path ; agent reach center plaza ใน ≤ 8s ทุก city |
| TR-WMS-041 (R15 Monument ambient pulse) | **Integration** | **Given** city_astra ไม่มี Fragment Event active **When** observe Monument visual state for 60s **Then** Monument emit `ambient_pulse` signal at regular interval (≥ 1 pulse/10s) ; intensity = ambient (ไม่ใช่ event-active) |
| TR-WMS-042 (R16 acknowledge primitive) | **Integration** | **Given** player A (faction_a) + player B (faction_b) ใน radius `R16_ACKNOWLEDGE_RADIUS_UNIT` (8 unit) **When** A กด acknowledge บน B **Then** ทั้งสองฝั่ง trigger reciprocal salute emote ภายใน 200ms ; ไม่มี chat message, ไม่มี party UI, ไม่มี friend/M11 event ; Player Record delta = ∅ ; visual emote ใช้สี both faction badges |
| TR-WMS-043 (R16 cooldown) | Logic | **Given** A เพิ่ง acknowledge B **When** A พยายาม acknowledge B อีกครั้งภายใน `R16_ACKNOWLEDGE_COOLDOWN_SECONDS` (60s) **Then** request rejected ; ไม่มี emote trigger ; client toast *"acknowledge cooldown active"* |
| TR-WMS-044 (R12 atomic rollback — failure injection) | **Integration** | **Given** Azure Function ส่ง `UpdatePlayerFt12State` request ที่ inject fault หลัง parse blob แต่ก่อน `SetObjects` PlayFab call **When** Function ส่งต่อ exception **Then** PlayFab Entity Object `ft12_player_state` ไม่เปลี่ยนแปลง (single-document atomicity จาก PlayFab native) ; player Record อ่านค่าเดิมทั้ง 4 fields ; client retry ด้วย same idempotency_key → no double-write |
| TR-WMS-045 (R12 idempotency) | Logic | **Given** client ส่ง same TravelRequest 3 ครั้งติด (network retry) ด้วย `idempotency_key` เดียวกัน **When** server process ทั้ง 3 requests **Then** request 1 commit + version++ ; request 2/3 detect `last_idempotency_key` match → no-op return cached result ; final state = single travel commit |
| TR-WMS-046 (D1 TOCTOU race) | **Integration** | **Given** 3 parties (party_size=5 each) co-locate-race ไป Astra_01 (pop=160, hard_ceiling=165) พร้อมกัน **When** ทั้ง 3 calls invoke `TryIncrementPopulation(astra_01, 5, 165)` พร้อมกัน **Then** เพียง 1 party ได้ success (atomic check-and-set) ; อีก 2 parties ได้ false → fall through ไป step 2 ; final pop ≤ 165 (ไม่เกิน hard_ceiling แม้ race) |
| TR-WMS-047 (EC-14 reconnect during EnteringCity) | Logic | **Given** player state = `EnteringCity`, network drop หลัง teleport request commit **When** client retry login ภายใน `ENTERING_HOLD_SECONDS` (60s) **Then** server hold instance slot ; reconnect → resume `EnteringCity` → complete to `InCity` ; instance assignment เดิม. **Given** retry > `ENTERING_HOLD_SECONDS` **Then** slot release + spawn ใหม่ตาม D1 |
| TR-WMS-048 (EC-18 EnteringCity timeout) | Logic | **Given** player state = `EnteringCity`, slow client load **When** elapsed time > `ENTERING_TIMEOUT_SECONDS` (30s) **Then** server force-fail load ; client return `OutOfUniverse` + error toast suggest retry ; instance slot release ; `current_population` decrement ถ้า slot เคย hold |
| TR-WMS-049 (EC-24 server-time authority) | Logic | **Given** client ส่ง chat message + countdown event ด้วย client-local timestamp ที่ skew +60s vs server UTC **When** server process **Then** stored timestamp = server UTC ทุก field (ไม่ใช่ client-supplied) ; client display time derive จาก server-issued event time + offset ; ไม่มี client-clock leak ใน stored state |
| TR-WMS-050 (R5.1 Galaxy Map interrupt rule) | **Integration** | **Given** player กำลัง select destination บน Galaxy Map **When** Fragment Event announce arrive ที่เมืองอื่น **Then** indicator pulse update **passive** (ไม่ modal, ไม่ auto-redirect) ; selection state ไม่เปลี่ยน ; ผู้เล่นต้อง confirm หรือ cancel ก่อน interact กับ event |
| TR-WMS-051 (EC-09 auto-blur) | Logic | **Given** player พิมพ์ใน text input (focus active) **When** match-found signal arrive **Then** text input auto-blur + current text auto-save ภายใน 100ms ; modal countdown ขึ้นทันที (ไม่ defer) ; player ไม่เสีย typed text |

### H.6 Manual Smoke (ADVISORY)

- **Manual-WMS-01 (B — first-visit fantasy):** QA login ด้วยบัญชีใหม่ที่ผ่าน FT11 faction selection แล้ว → เข้า World Map ครั้งแรก → verify: (1) ระฆังเมืองดัง, (2) ข้อความ environmental beat ปรากฏ (ไม่ใช่ tooltip popup), (3) ข้อความ contains exact provisional string *"ลานนี้เป็นดินแดนกลาง — ผู้ถือธงทุกฝ่ายแวะพักที่นี่เสมอกัน"* OR writer-pass replacement string ที่ approved ใน narrative review, (4) ไม่มี texture pop-in ขณะ beat play (EC-27 grace period), (5) functional orientation 2nd beat ปรากฏหลัง tone beat 3–5s (Keeper NPC location indicator + Galaxy Map hotkey hint). Lead sign-off required สำหรับ tone judgment ที่เกินเช็คลิสต์
...
### OQ-2 — Theme เฉพาะตัวของ 10 cities **(MILESTONE — converted from open question /design-review #2 finding N2)**
- **Primary Owner:** Narrative Director (named, not committee)
- **Supporting:** World Builder + Art Director (review/approve)
- **Deliverable:** **City Theme Brief** — 1 paragraph per city ใน
  `design/narrative/cities/[city-slug].md`: geographic archetype,
  cultural reference, emotional register, 1 signature sound, 1
  signature visual motif. R1 neutrality test ต้องผ่านก่อน commit
- **Sequencing:** Narrative Director author Brief → Art Director review
  → World Builder expand เป็น full world-building entry (Core Concept,
  Rules, History, Connections, Player Relevance, Contradictions Check)
- **Exit criteria:** ทั้ง 10 entries อยู่ใน `design/narrative/cities/`
  + Art Bible faction archetype section reference 10 entries
- **Trigger date:** ก่อน Asset Spec phase ; ก่อน R13 sharding ADR
- **Impact:** R3 (Anchor Content design), Visual/Audio (theme-bound
  bell sound, ambient soundscape), EC-22 (ambient NPC design), Keeper
  NPC theme variant, Art production blocking
- **Cross-ref:** อ้างถึงใน [game-concept.md](game-concept.md) Open Questions

### OQ-3 — Anchor Content รายการสมบูรณ์ของ 10 cities **(MILESTONE)**
- **Primary Owner:** Narrative Director (named) ; Game Designer
  consults R3 R1-alignment rule
- **Deliverable:** Anchor Content list (1 per city) + vignette pool
  ≥ 8 per Anchor (80 vignettes minimum at launch). Output ใน
  `design/narrative/cities/[city-slug].md` ภายใต้ Anchor section
- **Constraint:** ทุก Anchor ผ่าน R1 neutrality test (R3 quote:
  "Anchor นี้มีความหมายเท่าเทียมกันสำหรับผู้เล่นที่เพิ่งเปลี่ยนฝ่าย
  เมื่อวานหรือไม่?"). ห้าม faction-coded content
- **Trigger:** หลัง OQ-2 (themes) lock ; narrative team allocation
  confirmed (80 vignettes คือ scope ต้องระบุ staffing คาดว่า producer
  + narrative-director assess)
- **Exit criteria:** 10 Anchor entries + 80 vignettes committed ; CBS
  validation pass `ANCHOR_VIGNETTE_POOL_MIN=8`
- **Impact:** R3 detail ; UI surface count (Anchor screen ต่อ city) ;
  R3.1 rotation viability
...
### ~~OQ-9~~ — Cross-faction stranger interaction affordance **(RESOLVED — see R16 /design-review #2 finding I3+F8)**
- **Resolution:** Converted from open question to design commit ใน
  R16 (Cross-Faction Stranger Acknowledge Primitive). game-designer +
  ux-designer ทั้งคู่ระบุว่า OQ-9 เป็น Section B fantasy-load-bearing,
  defer ไป UX spec ทำลาย fantasy contract. R16 commits design intent
  ที่ GDD level ; UX spec รับเฉพาะ implementation form (trigger UI,
  animation spec, audio cue, accessibility input model)
- **R16 contract summary:** 1-button targeted acknowledge action ;
  ...
  distinct ; 60s cooldown per target ; opt-out via M11 emote settings
