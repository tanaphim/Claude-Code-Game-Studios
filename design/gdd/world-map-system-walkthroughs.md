...
- [x] **Phase 3 narrative pass closed (2026-05-05)** — narrative-director session: R8 avatar look-up cue (closes Cluster 1 #1), Keeper voice profile + 4 sample lines as archetype across 10 cities (closes Cluster 8 #17 + narr-1), EC-11 Philosophy B with 3-case Keeper-voiced ban broadcast (closes narr-2), OQ-2 theme spec template + city_01 + city_solis full briefs + producer notification for 8 remaining cities (closes narr-3 partial), R16 "Wayfarer's Nod" replaces "Traveler's Salute" (closes recommended item). 5 BLOCKING closed. Total blocker count 11 → 6.
- [x] **Phase 3 UX pass closed (2026-05-05)** — ux-designer session: R3.1 freshness signal with engineering-fork (Path A telemetry-derived vs Path B separate PlayFab key, R12 contract preserved both paths) (closes Cluster 4 #10) ; R15 pull mechanics 3 binding rules — sightline-from-spawn, ambient NPC social proof, queue-wait visual emphasis (closes Cluster 4 #11) ; EC-09 per-field-type draft fate — Option C loadout-name + Option B friend-search + IME limitation flagged (closes Cluster 8 #18). TR-WMS-059/060/061/062 added. 3 BLOCKING closed. Total blocker count 6 → 3. AC suite 58 → 62.
- [x] **Phase 3 Producer decision closed (2026-05-05)** — OQ-7 platform locked to Option A (PC desktop only, mouse+keyboard primary, gamepad+touch+console+mobile out-of-scope for MVP). Closes Cluster 8 #19. Patches: OQ-7 entry RESOLVED + UI Requirements platform notes + R2.1 hotkey description + Section G accessibility item #3 + `.claude/docs/technical-preferences.md` Input & Platform section locked. 1 BLOCKING closed. Total blocker count 3 → 2.
- [ ] **Phase 3 GDD revision effectively COMPLETE** — remaining 2 blockers not actionable in FT12 alone: Cluster 5 #13 (gated on FT13/FT14 GDD authoring), narr-3 8-cities (narrative writer-days allocation, ~4 wd, deferred to next sprint). Engineering pick (Path A vs B) for #10 state model also pending separate engineering session.
