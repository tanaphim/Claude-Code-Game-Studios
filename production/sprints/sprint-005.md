...
| Task | เหตุผล |
|------|--------|
| **S5-11 R-21 Item Role Restriction decision (1.0d) — DEFERRED to S6 Day-1** | Sprint 005 Phase 2 closeout consumed Should Have capacity; deferred to Sprint 006 Day-1 (1st carryover, ahead of pattern violation threshold but should not slip again). **Decide before or alongside S5-12** — S5-12 Path A depends on S5-11 outcome (Bot item priority logic differs per path). Briefing prepared at `production/sprint-006-prep/S5-11-r-21-item-role-restriction-briefing.md` with Path A (implement gating ~1.0d) / Path B (remove dead field ~0.35d, MVP-friendly) / Path C (soft hint badge ~0.5d, ambiguity risk). Closes risk register R-21 + Sprint 004 retro action #2. |
| **S5-12 AI Bot fate decision (0.5d) — DEFERRED WITH HARD COMMITMENT** | Sprint 005 Phase 2 closeout (S5-21 + S5-22) consumed planning capacity; decision deferred to Sprint 006 Day-1. **This is the 4th carryover** of the AI Bot work — Sprint 003 retro action #5 mandates no further defer. Briefing prepared at `production/sprint-006-prep/S5-12-ai-bot-fate-briefing.md` with Path A/B/C + dependency map + creative-director discussion prompts. Sprint 006 plan must close S5-12 before any other Should/Nice Have work starts. **Coordinate with S5-11** — decide S5-11 first or in the same conversation. |
| S4-09 AI Bot item-buying (2.0d) | depends on S5-12 path decision; if Path B → moves to `production/backlog/post-launch.md` |
| S4-10 AI Bot Difficulty (1.0d) | depends S4-09; if Path B → moves to post-launch backlog |
